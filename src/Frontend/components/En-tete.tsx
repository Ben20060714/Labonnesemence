        ...
        >
          <div className="px-3 py-4 space-y-1.5">
            {ongletsNavigation.map((onglet) => {
              const estActif = pageActive === onglet.cle;
              return (
                <button key={onglet.cle} id={`onglet-mobile-${onglet.cle}`} onClick={() => executerChangementPage(onglet.cle)} className={`w-full text-left px-4 py-3 rounded-lg text-sm font-semibold transition-all duration-150 cursor-pointer ${estActif? 'bg-[#c29f63]/10 text-[#af894d] dark:bg-[#c29f63]/25 dark:text-[#c3a064]': 'text-slate-600 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-800'}`}>
                  {onglet.libelle}
                </button>
              );
            })}
