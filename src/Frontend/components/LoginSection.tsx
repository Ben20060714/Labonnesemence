  ...
  const handleLogin = (e: SyntheticEvent) => {
    ...
    setTimeout(() => {
      if (email === 'test@example.com' && password === 'password') {
        ...
        // In a real app, you would handle token, user state, and redirection here
      } else {
    ...
    <section id="login-screen" className="flex items-center justify-center min-h-[calc(100vh-200px)] px-4 py-12 sm:px-6 lg:px-8">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-xl shadow-lg p-8 space-y-8 border border-[#f4ebd9]/60 dark:border-slate-800">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button type="submit" disabled={loading} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#af894d] hover:bg-[#936f3c] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#af894d] transition-colors duration-200">
