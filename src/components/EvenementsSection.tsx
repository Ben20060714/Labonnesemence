    ...
    <section id="calendrier-paroissial-screen" className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8 space-y-12">
            ...
            return (
              ...
              >
                ...
                <div className="px-6 pb-6 pt-2">
                  <button
                    id={`bouton-inscription-evt-${evt.identifiant}`}
                    onClick={() => definirEvenementAInscrire(evt)}
                    disabled={placesEpuisees}
                    className={`w-full py-3 rounded-md text-xs font-bold uppercase tracking-widest text-center shadow-sm transition-all cursor-pointer ${
                      placesEpuisees
                        ? 'bg-slate-105 text-slate-400 border border-slate-150 cursor-not-allowed dark:bg-slate-800 dark:text-slate-600 dark:border-slate-850'
                        : 'bg-[#af894d] text-white hover:bg-[#936f3c]'
                    }`}
                  >
                    {placesEpuisees ? 'Complet' : 'Réserver ma place'}
                  </button>
                </div>
      ...
      <div id="bloc-assistance-nouveaux" className="bg-[#fbf9f4] border border-[#f4ebd9] p-8 rounded-xl flex flex-col md:flex-row items-center justify-between gap-6 dark:bg-slate-900/40 dark:border-slate-800">
        <div className="space-y-2 text-left md:max-w-xl">
          ...
          <p className="text-sm text-slate-600 font-light leading-relaxed dark:text-slate-400">
            N’ayez aucune inquiétude : nos bénévoles de l’équipe d'accueil (protocoles) se feront une joie de vous escorter depuis l'arrêt, de vous présenter le déroulement des activités.
          </p>
        </div>
        <div className="flex items-center gap-2 text-[#af894d] text-xs uppercase font-mono tracking-widest font-bold">
          <Bell className="w-4 h-4 animate-bounce" /> Soyez le(la) bienvenue !
        </div>
      </div>
        ...
        {evenementAInscrire && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              onClick={() => definirEvenementAInscrire(null)}
            />

            {/* Corps de la Modale */}
            <motion.div
              id="corps-modale-inscription"
              initial={{ scale: 0.95, opacity: 0, y: 30 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.95, opacity: 0, y: 30 }}
              className="relative w-full max-w-lg bg-white rounded-xl shadow-2xl p-6 sm:p-8 space-y-6 text-slate-800 z-10 dark:bg-slate-900 dark:text-slate-100"
            >
              {/* Bouton Fermeture */}
              <button
                id="bouton-ferme-modale"
                onClick={() => definirEvenementAInscrire(null)}
              ...
              {!inscriptionTerminee ? (
                <form
                  id="formulaire-inscription-evenement"
                  onSubmit={soumettreFormulaireInscription}
                  className="space-y-4"
                >
                  <div className="space-y-1.5 text-left">
                    ...
                    <input
                      ...
                      value={nomDonneur}
                      onChange={(e) => definirNomDonneur(e.target.value)}
                      className="w-full px-4 py-3 text-sm rounded bg-slate-50 border border-slate-200 outline-none focus:border-[#af894d] focus:ring-1 focus:ring-[#af894d] dark:bg-slate-800 dark:border-slate-800 dark:text-slate-100"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5 text-left">
                    ...
                    <input
                      ...
                      id="champ-email"
                      required
                      placeholder="jean.dupuis@alliance.fr"
                      value={courrielDonneur}
                      onChange={(e) => definirCourrielDonneur(e.target.value)}
                      className="w-full px-4 py-3 text-sm rounded bg-slate-50 border border-slate-200 outline-none focus:border-[#af894d] focus:ring-1 focus:ring-[#af894d] dark:bg-slate-800 dark:border-slate-800 dark:text-slate-100"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5 text-left">
                    ...
                    <select
                      ...
                      onChange={(e) => definirPlacesReservees(Number(e.target.value))}
                      className="w-full px-4 py-3 text-sm rounded bg-slate-50 border border-slate-200 outline-none focus:border-[#af894d] focus:ring-1 focus:ring-[#af894d] dark:bg-slate-800 dark:border-slate-800 dark:text-slate-100"
                    >
                      {[1, 2, 3, 4, 5].map((idx) => (
                  ...
                  <div className="pt-3">
                    <button
                      type="submit"
                      id="bouton-valider-inscription-form"
                      className="w-full py-3.5 bg-[#af894d] hover:bg-[#936f3c] text-white font-bold text-xs uppercase tracking-widest rounded-md cursor-pointer transition-all"
                    >
                ...
                >
                  ...
                  <h4 className="font-serif text-2xl font-bold text-slate-900 dark:text-slate-100">
                    S'inscription confirmée !
                  </h4>
                  <p className="text-sm text-slate-500 font-light dark:text-slate-400 max-w-sm mx-auto">
                    Merci <span className="font-semibold text-slate-800 dark:text-slate-100">{nomDonneur}</span>. Un ticket numérique d'accès a été envoyé à <span className="font-mono text-slate-700 dark:text-slate-305">{courrielDonneur}</span>. À très bientôt pour vivre cette communion chrétienne !
                  </p>
                </motion.div>
              )}
            </motion.div>
